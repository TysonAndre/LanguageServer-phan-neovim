if ! exists('g:phan_analyzed_directory')
	...
	try
		...
		call LanguageClient_registerServerCommands({'php':[ g:phan_php_binary, s:ls, '--require-config-exists', '--language-server-on-stdin', '--quick', '--use-fallback-parser', '--allow-polyfill-parser', '--project-root-directory', g:phan_analyzed_directory]})
		" LanguageClientStart
	catch
		" do nothing
	endtry
